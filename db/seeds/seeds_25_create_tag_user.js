...
exports.seed = function(knex, Promise) {
  ...
  randomTagForTagUser = (array, user_id, promiseArr) => {
    ...
    tags.forEach((tag_id) => {
      promiseArr.push(knex('tag_user').insert(
        {
          tag_id: tag_id,
          user_id: user_id
        })
  ...
  users.forEach((user_id) => {
    randomTagForTagUser(colorTags, user_id, promiseArr);
    randomTagForTagUser(otherTags, user_id, promiseArr);
    randomTagForTagUser(styleTags, user_id, promiseArr);
    randomTagForTagUser(materialTags, user_id, promiseArr);
    randomTagForTagUser(brandTags, user_id, promiseArr);
  });

  promiseArr.push(knex('tags').orderBy('id'));
